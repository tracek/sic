...
def move_storage_class(
    ...
    try:
        ...
        if versioning_enabled:
            logger.info(f"Bucket '{bucket}' has versioning ENABLED")
            logger.info(
                "NOTE: Changing storage class in a versioned bucket creates new versions. "
                "If you don't delete old versions (--handle-versions delete), previous versions with "
                "original storage class will be retained as non-current versions "
                "and will continue to incur storage costs."
            )
        else:
            logger.info(f"Bucket '{bucket}' does not have versioning enabled or versioning is suspended")
            if handle_versions == "delete":
                logger.warning("Option --handle-versions delete is specified but bucket doesn't have versioning enabled. This option will have no effect.")
    except botocore.exceptions.ClientError as e:
        logger.warning(f"Could not check bucket versioning status: {e}")
    ...
    def move_batch(objects):
        ...
        Returns:
            Tuple of (success_count, list_of_failed, dict_of_version_ids)
        """
        success_count = 0
        failed_objects = []
        old_version_ids = {}  # Map of keys to their old version IDs
        ...
        for obj in objects:
            ...
            try:
                # If we need to delete old versions, fetch current version first
                if versioning_enabled and handle_versions == "delete":
                    try:
                        head_response = s3.head_object(Bucket=bucket, Key=key)
                        if 'VersionId' in head_response:
                            old_version_ids[key] = head_response['VersionId']
                    except botocore.exceptions.ClientError:
                        # If object doesn't exist, just continue
                        pass
                ...
                # If we need to delete old versions and we have a version ID, we'll do it after
        ...
        return success_count, failed_objects, old_version_ids
    
    # Function to delete a batch of old versions
    def delete_old_versions(version_map):
        """
        Delete a batch of old object versions.
        
        Args:
            version_map: Dictionary mapping keys to their old version IDs
        ...
        # Prepare batches of 1000 objects (S3 limit)
        objects_to_delete = []
        
        for key, version_id in version_map.items():
            objects_to_delete.append({
                'Key': key,
                'VersionId': version_id
            })
            
            # If we have a full batch, delete it
            if len(objects_to_delete) >= batch_size:
                try:
                    ...
                    if 'Errors' in response:
                        ...
                        for error in response['Errors']:
                            logger.warning(f"Failed to delete version: {error.get('Key')} (version: {error.get('VersionId')}): {error.get('Code')} - {error.get('Message')}")
                    
                except Exception as e:
                    logger.error(f"Error deleting batch of versions: {e}")
    ...
    try:
        ...
        total_failed = 0
        total_skipped = 0
        all_version_ids = {}  # To track old versions for deletion
        ...
        # If no input file but prefix is provided, list objects from S3
            ...
            for page in tqdm(paginator.paginate(Bucket=bucket, Prefix=prefix),
                            desc="Listing objects", unit="batch"):
                for obj in page.get('Contents', []):
                    # Skip if object is already in target storage class
                    if obj.get('StorageClass') == storage_class:
                        logger.debug(f"Skipping {obj['Key']} - already in {storage_class} storage class")
                        total_skipped += 1
                        continue
        ...
        if len(objects_to_move) > 0:
            ...
            for i in range(sample_size):
                ...
                storage_class_str = f" [from: {obj.get('StorageClass', 'UNKNOWN')}]" if 'StorageClass' in obj else ""
                logger.info(f"  {obj['Key']}{size_str}{storage_class_str}")
        ...
        if dry_run:
            ...
            if versioning_enabled and handle_versions == "delete":
                logger.info(f"DRY RUN: Would also delete the previous versions of objects")
            return
        
        # Provide information about versioning and potential costs
        if versioning_enabled:
            if handle_versions == "delete":
                logger.info(
                    "The old versions will be deleted after changing storage class to minimize storage costs. "
                    "This ensures only the new version in the target storage class is kept."
                )
            elif handle_versions == "keep":
                logger.warning(
                    "You've chosen to keep old versions. Be aware that after changing the storage class, "
                    "both the new version (in the target storage class) and previous versions will exist "
                    "and incur storage costs."
                )
        
        # Confirm before proceeding
        if not no_confirm:
            confirm_msg = f"Are you sure you want to move {len(objects_to_move)} objects to {storage_class} storage class?"
            if versioning_enabled and handle_versions == "delete":
                confirm_msg += " Previous versions will be deleted."
        ...
        with tqdm(total=len(objects_to_move), desc=f"Moving to {storage_class}", unit="obj") as progress:
            for i in range(0, len(objects_to_move), batch_size):
                ...
                # Process the batch
                success, failed, version_ids = move_batch(batch)
                ...
                # Track version IDs for later deletion if needed
                all_version_ids.update(version_ids)
                
                # Update progress
                progress.update(len(batch))
        
        # Delete old versions if requested
        if versioning_enabled and handle_versions == "delete" and all_version_ids:
            logger.info(f"Deleting {len(all_version_ids)} previous versions...")
            
            with tqdm(total=len(all_version_ids), desc="Deleting old versions", unit="ver") as progress:
                # Process in smaller batches to track progress
                batch_size_ver = min(batch_size, 100)  # Smaller batches for better progress
                
                for i in range(0, len(all_version_ids), batch_size_ver):
                    # Extract batch of version IDs
                    batch_keys = list(all_version_ids.keys())[i:i+batch_size_ver]
                    batch_versions = {k: all_version_ids[k] for k in batch_keys}
                    
                    # Delete the batch
                    success_ver, failed_ver = delete_old_versions(batch_versions)
                    
                    # Update counters
                    total_versions_deleted += success_ver
                    total_versions_failed += failed_ver
                    
                    # Update progress
                    progress.update(len(batch_versions))
        ...
        if total_skipped > 0:
            logger.info(f"Skipped (already in target class or filtered out): {total_skipped}")
        
        if total_failed > 0:
            logger.warning(f"Failed to move: {total_failed}")
        
        if versioning_enabled and handle_versions == "delete":
            logger.info(f"Previous versions deleted: {total_versions_deleted}")
            if total_versions_failed > 0:
                logger.warning(f"Failed to delete versions: {total_versions_failed}")
        
        # Provide lifecycle rule advice if appropriate
        if versioning_enabled and handle_versions == "advise":
            logger.info("\nLIFECYCLE RULE RECOMMENDATION:")
            logger.info(
                "To minimize storage costs, consider setting up a lifecycle rule in your bucket to:"
                "\n1. Transition non-current versions to cheaper storage classes after a period"
                "\n2. Expire (delete) non-current versions after your required retention period"
                "\n3. Or use lifecycle rules directly instead of this command for storage transitions"
            )
            logger.info(
                "Example AWS CLI command to create a lifecycle rule:"
                "\n"
                "\naws s3api put-bucket-lifecycle-configuration --bucket " + bucket + " --lifecycle-configuration file://lifecycle.json"
                "\n"
                "\nWith lifecycle.json containing:"
                "\n{"
                '\n  "Rules": ['
                '\n    {'
